window.__ModuleLoader__.load({ id: "dsh-purge", factory: (require) => {
		...
		const zh = {
			...
			"apply.dshaRequired": "DSHA 启动时不会自动应用补丁，请点「应用」后再从 DSHA 启动页重启 Web。若重启后 Web 起不来，请在 DSHA 的启动恢复页停用本插件。",
		...
		const en = {
			...
			"apply.dshaRequired": "DSHA does not auto-apply at startup. Press Apply, then restart Web from DSHA's Launch page. If Web then fails to start, use DSHA's recovery page to disable this plugin.",
...
`;
		...
		function PurgifySection() {
			...
			return h("section", { className: "dshp-panel", "aria-label": t("purge.title") },
				...
				hostSurface === "dsha" ? h("p", { className: "dshp-hint", style: { margin: "8px 0 0", color: "var(--dshp-mute)", fontSize: 12 } }, hostText(t, "apply.dshaRequired", "dsha")) : null,
