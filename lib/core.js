...
export function applyRuntimeEnv() {
  ...
  // 这里曾经在 DSH_PERMISSION_MODE === "danger-full-access" 时 delete 该变量。
  // 那会让 dsh-base 的 permission 行读到 undefined：
  //   sandbox-policy.mode / approval.policy 都靠 process.env.DSH_PERMISSION_MODE 求值，
  //   两边一旦落到不同的 ?? 兜底值，composed (sandbox, approval) 就对不上
  //   dsh-permission-presets 的 presets 表（read-only/workspace-write/danger-full-access），
  //   PermissionPresetService 随即抛 "composed sandbox and approval defaults match
  //   no preset"，@deepseek-ai/dsh-base 加载失败，整个 Web 起不来。
  // 本函数只负责「汇报当前环境」，不得改动进程环境；真正的降级交由 YAML 默认值处理。
  return {
...
export function listShimDirsForPatch() {
  // Desktop and DSHA own their launchers; patching those shims bypasses host
  // lifecycle/PID contracts. Only the standalone official Web CLI is patched.
  if (adapterFor(detectSurface()) !== "web") return [];
  return collectDirs([
    findShimDir(),
