...
function shimNeedsPatch(state) {
  if (hostctl.currentSurface() !== "web") return false;
  return (
...
async function autoApply(config) {
  try {
    // DSHA loads the Web profile while this plugin is starting. Rewriting the
    // base permission YAML here can expose the loader to the intermediate
    // mode=full/policy=ask combination before the paired policy patch lands.
    // Let the user Apply with the app's guard in place, then restart Web from DSHA.
    if (hostctl.currentSurface() === "dsha") {
      log(config, "skip auto-apply: DSHA startup must not rewrite the live loader tree");
      return "skip:dsha_manual_apply_required";
    }
        ...
        handler: async (request, response) => {
          ...
          try {
            ...
            sendJson(response, result.ok ? 200 : 500, {
              ...
              hostManaged: hostctl.currentSurface() === "dsha",
