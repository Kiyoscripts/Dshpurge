...
export async function applyUpdate(jobs = planUpdates(), options = {}) {
  ...
  // DSHA owns plugin install/update for the container (its plugin manager
  // rewrites the profile, links the package and restarts Web). Updating from
  // inside the container would race that transaction.
  if (detectSurface() === "dsha") {
    throw new Error("DSHA manages plugin updates. Update dsh-purge from DSHA's Plugins page.");
  }
  const state = readChannelState();
...
export async function autoUpdateIfNeeded() {
  try {
    if (detectSurface() === "dsha") {
      return rememberUpdate({ ok: true, auto: true, hasUpdate: false, skipped: "host_managed" });
    }
